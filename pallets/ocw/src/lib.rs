...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
        ...
        #[pallet::weight(10000)]
		pub fn submit_mini_remote_block(origin: OriginFor<T>, address: Vec<u8>, energyacum: u32) -> DispatchResult {
			let who = ensure_signed(origin)?;
			Self::set_remote_block(address.clone(),energyacum);
			Ok(())
		}
	}
...
impl<T: Config> Pallet<T> {
    ...
    fn set_remote_block(address: Vec<u8>,power: u32) {
    ...
    fn fetch_and_parse_data() -> Result<(), http::Error> {
		...
		let energyacum = match Self::parse_energyacum(body_str) {
			Some(energyacum) => Ok(energyacum),
			None => {
		...
		log::info!("Got energyacum: {} ", energyacum);
    ...
    fn parse_energy(data_str: &str) -> Option<u32> {
    ...
    fn parse_gridcode(data_str: &str) -> Option<Vec<u8>> {
		...
		let devices = match val.ok()? {
			JsonValue::Object(obj) => {
				let (_, v) = obj.into_iter().find(|(k, _)| k.iter().copied().eq("gridcode".chars()))?;
				match v {
	...
	fn parse_energyacum(data_str: &str) -> Option<u32> {
		let val = lite_json::parse_json(data_str);
		let power = match val.ok()? {
			JsonValue::Object(obj) => {
				let (_, v) = obj.into_iter().find(|(k, _)| k.iter().copied().eq("energyacum".chars()))?;
				match v {
					JsonValue::Number(number) => number,
					_ => return None,
				}
