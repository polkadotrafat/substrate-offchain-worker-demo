...
pub mod pallet {
	...
	#[derive(Deserialize, Encode, Decode, Clone, Eq, PartialEq, Default, RuntimeDebug, scale_info::TypeInfo)]
	struct MiniGrid {
		// Specify our own deserializing function to convert JSON string to vector of bytes
		block: u32,
		devices: u32,
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			let result = Self::fetch_n_parse_grid_data();
			...
			//	3 => Self::fetch_n_parse_grid_data(),
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10000)]
		pub fn submit_mini_remote_block(origin: OriginFor<T>,  block: u32, devices: u32) -> DispatchResult {
			let who = ensure_signed(origin)?;
			Self::set_remote_block(block,devices);
			Ok(())
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		fn set_remote_block(block: u32, devices: u32) {
		...
		fn fetch_n_parse_grid_data() -> Result<(), http::Error> {
			let resp_bytes = Self::fetch_grid_data().map_err(|e| {
			...
			//let info = lite_json::parse_json(resp_str).expect("Invalid JSON specified!");
			//serde_json::from_str(&resp_str).map_err(|_| <Error<T>>::DeserializeToObjError)?;

			let block = match Self::parse_block(resp_str) {
				Some(block) => Ok(block),
				None => {
					log::warn!("Unable to extract price from the response: {:?}", resp_str);
					Err(http::Error::Unknown)
				},
			}?;
	
			log::info!("Got price: {} cents", block);

			let devices = match Self::parse_block(resp_str) {
			...
			let result = signer.send_signed_transaction(|_acct|
				...
				//Call::submit_mini_grid_data { data }
				Call::submit_mini_remote_block { block,devices }
			);
		...
		fn parse_block(resp_str: &str) -> Option<u32> {
			let val = lite_json::parse_json(resp_str);
			let block = match val.ok()? {
				JsonValue::Object(obj) => {
					let (_, v) = obj.into_iter().find(|(k, _)| k.iter().copied().eq("blockNumber".chars()))?;
					match v {
		...
		fn parse_devices(resp_str: &str) -> Option<u32> {
			let val = lite_json::parse_json(resp_str);
		...
		fn fetch_grid_data() -> Result<Vec<u8>, http::Error> {
			// Initiate an external HTTP GET request. This is using high-level wrappers from `sp_runtime`.
			let request = http::Request::get(HTTP_GRID_REMOTE_REQUEST);
		...
		fn offchain_signed_tx(block: u32) -> Result<(), Error<T>> {
			...
			//let result = signer.send_signed_transaction(|_acct|
			//	// This is the on-chain function
			//	Call::submit_mini_remote_block { block }
			//);
