...
impl<T: Config> Pallet<T> {
	...
	fn fetch_mini_grid_data() -> Result<u32, http::Error> {
		let timeout = sp_io::offchain::timestamp().add(Duration::from_millis(3_000));

		let request = http::Request::get("http://localhost:3000/robonomics");
		...
		log::info!("fetch_mini_grid_data): {}", body_str);

		let val = lite_json::parse_json(body_str);

		//log::info!("fetch_mini_grid_data_parsed: {}", val);
